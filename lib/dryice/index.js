...
function addSourceBase(obj, baseObj) {
    var read = fs.readFileSync(pathJoin(baseObj.base, baseObj.path));
    obj.sources.push({
        name: baseObj,
...
(function() {
    ...
    function findModuleAt(baseObj, base, path) {
        ...
        if (/^\//.test(path)) {
            if (isFile(path)) {
                baseObj = { base: '/', path: path };
                return baseObj;
            }
        }

        if (isFile(pathJoin(base, path))) {
            if (baseObj) {
                console.log('- Found several matches for ' + module +
                    ' (ignoring 2nd)');
                console.log('  - ' + pathJoin(baseObj.base, baseObj.path));
                console.log('  - ' + pathJoin(base, path));
            }
            else {
    ...
    function relative(pathA, pathB) {
        ...
        pathA = pathA.slice(i).filter(Boolean);
        pathB = pathB.slice(i).filter(Boolean);
    ...
    function findRequires(baseObj) {
        var code = fs.readFileSync(pathJoin(baseObj.base, baseObj.path)).toString();
        var ast;
        try {
        ...
        var walkers = {
            'call': function(expr, args) {
                ...
                if (expr[1] === 'require') {
                    var arg0 = args[0];
                    if (args[0][0] === 'string') {
                        reply.push(/^\./.test(args[0][1])
                                   ? relative(baseObj.base,
                                              pathJoin(dirname(pathJoin(baseObj.base, baseObj.path)),
                                                       args[0][1]))
                                   : args[0][1]);
                    }
                    else {
